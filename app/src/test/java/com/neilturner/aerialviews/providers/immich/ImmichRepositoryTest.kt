...
internal class ImmichRepositoryTest {
    ...
    private fun serverVersionResponse(
        major: Int,
        minor: Int = 0,
    ): ServerVersionResponse = ServerVersionResponse(major = major, minor = minor, patch = 0)
    ...
    inner class GetServerVersion {
        @Test
        fun `returns major version from API`() =
            runTest {
        ...
        @Test
        fun `v1 with minor 118 is treated as v3`() =
            runTest {
        ...
        @Test
        fun `v1 with minor 105 is treated as v2`() =
            runTest {
        ...
        @Test
        fun `caches result after first call`() =
            runTest {
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(3))
        ...
        @Test
        fun `falls back to v2 on API error`() =
            runTest {
        ...
        @Test
        fun `falls back to v2 on network exception`() =
            runTest {
    ...
    inner class GetSharedAlbumFromAPI {
        @Test
        fun `returns INDIVIDUAL type shared link assets`() =
            runTest {
        ...
        @Test
        fun `returns ALBUM type with fetched album details`() =
            runTest {
                every { prefs.pathName } returns "share/12345"
                every { prefs.password } returns ""
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
                ...
                coEvery { api.getSharedAlbum(key = "12345", slug = null, password = null) } returns Response.success(sharedResponse)
                coEvery { api.getSharedAlbumById(albumId = "album-1", key = "resolved-key", password = null) } returns
                    Response.success(albumResponse)
        ...
        @Test
        fun `handles ALBUM type with null album`() =
            runTest {
                every { prefs.pathName } returns "share/12345"
                every { prefs.password } returns ""
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `handles unknown type with fallback`() =
            runTest {
                every { prefs.pathName } returns "share/12345"
                every { prefs.password } returns ""
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `throws exception on API error`() =
            runTest {
                every { prefs.pathName } returns "share/12345"
                every { prefs.password } returns ""
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `v3 - uses getSharedAlbumV3 (no password param)`() =
            runTest {
                every { prefs.pathName } returns "share/12345"
        ...
        @Test
        fun `v3 - fetches ALBUM assets via search metadata`() =
            runTest {
                every { prefs.pathName } returns "share/12345"
                every { prefs.password } returns ""
        ...
        @Test
        fun `v2 - falls back to search metadata when inline assets are empty`() =
            runTest {
                every { prefs.pathName } returns "share/12345"
                every { prefs.password } returns ""
                every { prefs.apiKey } returns ""
    ...
    inner class GetSelectedAlbumFromAPI {
        @Test
        fun `fetches and combines multiple albums (v2)`() =
            runTest {
        ...
        @Test
        fun `deduplicates assets across albums (v2)`() =
            runTest {
                every { prefs.selectedAlbumIds } returns mutableSetOf("album-1", "album-2")
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `returns empty album when no albums selected`() =
            runTest {
        ...
        @Test
        fun `throws exception when no assets found (v2)`() =
            runTest {
        ...
        @Test
        fun `falls back to search metadata when inline assets are empty (v2)`() =
            runTest {
                every { prefs.selectedAlbumIds } returns mutableSetOf("album-1")
                every { prefs.apiKey } returns "test-api-key"
                every { prefs.mediaType } returns ProviderMediaType.VIDEOS_PHOTOS
        ...
        @Test
        fun `continues with other albums when one fails (v2)`() =
            runTest {
                every { prefs.selectedAlbumIds } returns mutableSetOf("album-1", "album-2")
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `v3 - paginates album asset fetching`() =
            runTest {
                every { prefs.selectedAlbumIds } returns mutableSetOf("album-1")
                every { prefs.apiKey } returns "test-api-key"
                every { prefs.mediaType } returns ProviderMediaType.VIDEOS_PHOTOS
    ...
    inner class GetFavoriteAssetsFromAPI {
        @Test
        fun `fetches favorites with count limit`() =
            runTest {
        ...
        @Test
        fun `returns empty list when count is empty`() =
            runTest {
    ...
    inner class GetRatedAssetsFromAPI {
        @Test
        fun `fetches assets for multiple ratings`() =
            runTest {
        ...
        @Test
        fun `returns empty list when no ratings configured`() =
            runTest {
        ...
        @Test
        fun `continues when one rating fails`() =
            runTest {
    ...
    inner class GetRandomAssetsFromAPI {
        @Test
        fun `fetches random assets with count`() =
            runTest {
    ...
    inner class GetRecentAssetsFromAPI {
        @Test
        fun `fetches recent assets with count`() =
            runTest {
    ...
    inner class FetchAlbums {
        @Test
        fun `fetches and combines regular and shared albums (v2)`() =
            runTest {
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `deduplicates albums by ID (v2)`() =
            runTest {
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `returns failure when regular albums fetch fails (v2)`() =
            runTest {
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `continues when shared albums fetch fails (v2)`() =
            runTest {
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `v3 - uses isShared param instead of shared`() =
            runTest {
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(3))
        ...
        @Test
        fun `v3 - returns failure when regular albums fetch fails`() =
            runTest {
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(3))
        ...
        @Test
        fun `network timeout escapes coroutineScope - requires Fragment handler`() =
            runTest {
                every { prefs.apiKey } returns "test-api-key"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
        ...
        @Test
        fun `trims whitespace from API key before calling API`() =
            runTest {
                every { prefs.apiKey } returns "test-api-key\n"
                coEvery { api.getServerVersion() } returns Response.success(serverVersionResponse(2))
